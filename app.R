...
server <- function(input, output, session) {
    ...
    output$downloadBiplot <- downloadHandler(
        ...
        content = function(file) {
            ...
            ggsave(file, plot = output$biplotPlot(), device = "png", width = 10, height = 8, units = "in", dpi = 300)
        }
    )
}
