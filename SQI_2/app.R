...
# install.packages(c("shiny", "shinydashboard", "readr", "dplyr", "FactoMineR", "ggplot2", "DT", "forcats", "tibble", "tidyr", "factoextra", "agricolae", "multcompView", "rlang"))
            ...
            tabItem(tabName = "results",
                    ...
                    fluidRow( # New row for the grouping variable selection and bar graph
                        box(
                            ...
                            # New dropdown for post-hoc test selection
                            selectInput("post_hoc_test", "Select Post-Hoc Test:",
                                        choices = c("LSD (with Bonferroni)" = "LSD",
                                                    "Tukey's HSD" = "Tukey"),
                                        selected = "Tukey"),
...
server <- function(input, output, session) {
    ...
    calculated_sqi_data <- eventReactive(input$calculate_sqi, {
        ...
        temp_sqi_calc_df <- scores_df_numeric_only %>%
            select(all_of(paste0(mds_vars, "_Score")))
        ...
        # Combine original data with calculated SQI and individual scores
        all_final_cols <- df_original %>%
            bind_cols(scores_df_numeric_only %>% select(starts_with(paste0(mds_vars, "_Score")))) %>%
            mutate(SQI = temp_sqi_calc_df$SQI_temp * 100) # Scale to 0-100
    ...
    # Render Grouped SQI Bar Plot with CLD
    output$sqi_group_bar_plot <- renderPlot({
        req(calculated_sqi_data()$sqi_data, input$post_hoc_test)
        ...
        # Check if there are at least two groups to perform the test
        if (n_distinct(plot_data[[group_var]]) < 2) {
            return(
                ggplot() +
                    geom_text(aes(x=0.5, y=0.5, label="Not enough groups to perform a post-hoc test.")) +
                    theme_void() +
                    labs(title = "SQI Grouped Bar Graph")
            )
        }
        
        # Calculate summary statistics and order the groups by mean
        summary_data <- plot_data %>%
            ...
            arrange(desc(Mean_SQI)) # Sort by mean SQI in descending order
        
        # Reorder the factor levels of the grouping variable based on the summary data
        plot_data[[group_var]] <- factor(plot_data[[group_var]], levels = summary_data[[group_var]])
        
        # Perform ANOVA
        aov_formula <- as.formula(paste("SQI ~", group_var))
        aov_model <- aov(aov_formula, data = plot_data)
        
        # Conditional Post-Hoc Test based on user input
        if (input$post_hoc_test == "Tukey") {
            ...
            subtitle_text <- "Groups sharing a letter are not significantly different (Tukey's HSD)"
            
        } else if (input$post_hoc_test == "LSD") {
            ...
            subtitle_text <- "Groups sharing a letter are not significantly different (Bonferroni-adjusted LSD)"
        }
        
        # Combine summary data with CLD data
        final_plot_data <- summary_data %>%
            left_join(cld_data, by = group_var) %>%
            mutate(y_cld = Mean_SQI + SD_SQI + 2.5)
        
        # Create the plot
        ggplot(final_plot_data, aes(x = !!sym(group_var), y = Mean_SQI, fill = !!sym(group_var))) +
            geom_bar(stat = "identity", color = "black") +
            geom_errorbar(aes(ymin = Mean_SQI - SD_SQI, ymax = Mean_SQI + SD_SQI), width = 0.2, position = position_dodge(.9)) +
            geom_text(aes(label = cld, y = y_cld), vjust = 0, size = 5) +
            labs(
                title = paste("Mean Soil Quality Index (0-100) by", group_var),
            ...
            annotate("text", x = Inf, y = 20, label = "Q4: Poor (20-40)", hjust = 1.1, vjust = -0.5, size = 3, color = "firebrick") +
            annotate("text", x = Inf, y = 0, label = "Q5: Extremely Poor (0-20)", hjust = 1.1, vjust = -0.5, size = 3, color = "darkred") +
